...
var xOrO = "X";
var turn = 0; 
var clickedBox, chosenBox, winner, holder; 
var unmarkedBoxes = selectedBoxes = boxGroupsIndex = filteredGroups = [];
...
function addClick(){
	for(var i = 0; i < block.length; i++){
		...
		block[i].addEventListener("click",function(i){
...
function clickBox(){
	for(var i = 0; i <block.length; i++){
		if(block[i].id == chosenBox){
			...
			console.log("ClickBox() " + clickedBox);
		}
	}
...
function updateBox(){
	...
	console.log("updateBox() " + boxGroupsIndex);
...
function updateGroups(){
	for(var i = 0; i < boxGroupsIndex.length; i++){
		if(turn == 0){
			...
			console.log("updateGroups() weight" + groups[boxGroupsIndex[i]].weight);
			console.log("updateGroups() counter" + groups[boxGroupsIndex[i]].counter);
...
function resetVariables(){
	if(winner ==null){
		clickedBox = chosenBox = "";
		unmarkedBoxes = selectedBoxes = boxGroupsIndex = filteredGroups = [];
...
function searchBoxInGroups(){
	...
	console.log("searchBoxInGroups() " + selectedBoxes);
...
function resetGame(){
	...
	unmarkedBoxes = selectedBoxes = boxGroupsIndex = filteredGroups = [];
	xOrO = turn = winner = holder = clickedBox = chosenBox = "";
	X.removeAttribute("disabled");
	O.removeAttribute("disabed");
